...
for iCounter in range(0, int(len(linSol_files)/4)):
    ...
    if iCounter == 0:
        ...
        ax1.text(-0.12, 0.98, 'Simulation Time [ms]', fontsize=14, fontweight='bold', transform=ax1.transAxes, rotation=90)

    elif iCounter == 1:
        ax1 = plt.axes([left + iCounter * row_factor, bottom, width, height])
        ax1.text(0.35, 1.05, 'linsol = GMRES', fontsize=14, fontweight='bold', transform=ax1.transAxes)
        ax1.get_xaxis().set_visible(False)
        ...
        ax1.get_yaxis().set_visible(False)
        ax1.text(0.15, -0.3, 'Number of state variables', fontsize=24, transform=ax1.transAxes)
...
# set global labels
plt.text(0.1, 4, 'Simulation time distribution of models for different linear solver combinations', fontsize=24, transform=ax1.transAxes)  # -60 , 350
