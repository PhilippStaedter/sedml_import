...
# important paths
model_base_path = './sedml_models/perelson1996_fig1b_top'
...
result = pypesto.minimize(problem=problem, optimizer=optimizer, n_starts=5, engine=engine)
...
plt.savefig(model_base_path + '/waterfall_estimation.pdf')
pypesto.visualize.parameters(result, reference=ref)
fig = plt.gcf()
fig.set_size_inches(18.5, 10.5)
plt.savefig(model_base_path + '/waterfall_parameter.pdf')
...
fig = plt.gcf()
fig.set_size_inches(18.5, 10.5)
plt.savefig(model_base_path + '/measurement.pdf')
df.to_csv(model_base_path + '/simulatedData_perelson1996_fig1b_top.tsv', sep='\t', index=False)