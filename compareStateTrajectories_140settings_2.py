...
def compStaTraj_JWS_2():
    ...
    if not os.path.exists(folder_path):
        print('Error: The folder Whole_state_trajectory_comparison should have been created by now!')
        sys.exit()
    ...
    for iSimDataGroup in list_simulation_data:
        ...
        solAlg, nonLinSol, linSol, absTol, relTol = iSimDataGroup.split('_')
        ...
        for iAbsError in range(0, len(AbsError_1)):
            for iRelError in range(0, len(RelError_2)):
                ...
                list_models_with_simulationData = sorted(os.listdir(folder_path + '/' + iSimDataGroup + '/json_files'))
                for iMod in range(0, len(list_models_with_simulationData)):
                    ...
                    list_files = sorted(os.listdir(folder_path + '/' + iSimDataGroup + '/json_files/' + iModel))
                    for iFile in list_files:
                        print(f"    {iModel} :: {iFile}")

                        # important paths
                        old_json_save_path = folder_path + '/' + iSimDataGroup + '/json_files/' + iModel + '/' + iFile
                        new_json_save_path = folder_path + '/' + f'{solAlg}_{nonLinSol}_{linSol}_{absTol}_{relTol}' \
                                             + '/all_results_json_files/json_files_' + abs_str + '_' + rel_str + '/' + iModel + '/' + iFile
