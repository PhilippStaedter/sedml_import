...
# create folder with all models that did not pass the trajectory check for the best setting 2_2_9_12_10
path_all_models = '../sbml2amici/amici_models_newest_version_0.10.19'
path_correct_trajectories = '../sbml2amici/correct_amici_models_paper'
path_not_correct_trajectories = '../sbml2amici/0.10.19_without_correct'
if not os.path.exists(path_not_correct_trajectories):
    amici = sorted(os.listdir(path_all_models))
    correct = sorted(os.listdir(path_not_correct_trajectories))
    for iModel in amici:
        if iModel not in correct:
            shutil.copytree(path_all_models + '/' + iModel, path_not_correct_trajectories + '/' + iModel)
        else:
            model_amici = sorted(os.listdir(path_all_models + '/' + iModel))
            model_correct = sorted(os.listdir(path_correct_trajectories + '/' + iModel))
            for iFile in model_amici:
                if iFile not in model_correct:
                    if not os.path.exists(path_not_correct_trajectories + '/' + iModel):
                        os.makedirs(path_not_correct_trajectories + '/' + iModel)
                    shutil.copytree(path_all_models + '/' + iModel + '/' + iFile, path_not_correct_trajectories + '/' + iModel + '/' + iFile)
                else:
                    continue
        ...
        for nonLinSol in iterative:
            ...
            for linSol in linearsol:
                ...
                for iTolerance in Tolerance_combination:
                    ...
                    _,rtol_exp = str('{:.1e}'.format(iTolerance[1])).split('-')
                    if len(atol_exp) != 2:
                        atol_exp = '0' + atol_exp
                    if len(rtol_exp) != 2:
                        rtol_exp = '0' + rtol_exp
                    ...
                    # get all models
                    list_directory_amici = sorted(os.listdir('../sbml2amici/0.10.19_without_correct'))
                    if delete_counter != 0:
                        ...
                        for iFile in list_files:
                            ...
                            if os.path.exists(BioModels_path + '/' + iModel):
                                ...
                                try:
                                    ...
                                    compStaTraj_JWS(iMod, solAlg, nonLinSol, linSol, iTolerance)
