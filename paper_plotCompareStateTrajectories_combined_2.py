...
all_log_abs_tol = ['03', '06', '06', '16', '12', '14']
all_log_rel_tol = ['03', '03', '06', '08', '10', '14']
all_abs_tol = [r'$10^{-3}$', r'$10^{-6}$', r'$10^{-6}$', r'$10^{-16}$', r'$10^{-12}$', r'$10^{-14}$']
all_rel_tol = [r'$10^{-3}$', r'$10^{-3}$', r'$10^{-6}$', r'$10^{-8}$', r'$10^{-12}$', r'$10^{-14}$']
...
for iTolerance in range(0, len(all_log_abs_tol)):
    ...
    for Multistep in ['1', '2']:
        ...
        for tol_exp in range(-20, 10):
            ...
            base_dir_JWS = f"FigS1_preselection/all_results_{Multistep}_{nonLinSol}_{linSol}_{abs_tol}_{rel_tol}/json_files_{tol_str}_{tol_str}"
            sedml_models_JWS = sorted(os.listdir(base_dir_JWS))
            base_dir_COPASI = f"FigS1_preselection/all_results_COPASI_{Multistep}_{nonLinSol}_{linSol}_{abs_tol}_{rel_tol}/json_files_{tol_str}_{tol_str}"
            ...
            for sedml_model in sedml_models_JWS:
                ...
                for sbml_model in sbml_models:
                    ...
                    if df['trajectories_match'][0] == True:
                        if iTolerance == 0:
                            list_1.append(sedml_model + '_' + sbml_model)
                        if iTolerance == 1:
                            list_2.append(sedml_model + '_' + sbml_model)
                        counter += 1
    ...
    if iTolerance == 0:
        ...
        counter_Tol_5.append(counters_Adams)
        counter_Tol_5.append(counters_BDF)

ax = plt.axes()
ax.plot(tol_exps, counter_Tol_0[0], '-*', c='#fee5d9', label=f'AM & {all_abs_tol[0]} & {all_rel_tol[0]}')
ax.plot(tol_exps, counter_Tol_0[1], '-*', c='#abd9e9', label=f'BDF & {all_abs_tol[0]} & {all_rel_tol[0]}')
ax.plot(tol_exps, counter_Tol_1[0], '-*', c='#fdae61', label=f'AM & {all_abs_tol[1]} & {all_rel_tol[1]}')
ax.plot(tol_exps, counter_Tol_1[1], '-*', c='#74add1', label=f'BDF & {all_abs_tol[1]} & {all_rel_tol[1]}')
ax.plot(tol_exps, counter_Tol_2[0], '-*', c='#f46d43', label=f'AM & {all_abs_tol[2]} & {all_rel_tol[2]}')
ax.plot(tol_exps, counter_Tol_2[1], '-*', c='#4575b4', label=f'BDF & {all_abs_tol[2]} & {all_rel_tol[2]}')
ax.plot(tol_exps, counter_Tol_3[0], '-*', c='#d73027', label=f'AM & {all_abs_tol[3]} & {all_rel_tol[3]}')
ax.plot(tol_exps, counter_Tol_3[1], '-*', c='#313695', label=f'BDF & {all_abs_tol[3]} & {all_rel_tol[3]}')
ax.plot(tol_exps, counter_Tol_4[0], '-*', c='#a50026', label=f'AM & {all_abs_tol[4]} & {all_rel_tol[4]}')
ax.plot(tol_exps, counter_Tol_4[1], '-*', c='#2E2D66', label=f'BDF & {all_abs_tol[4]} & {all_rel_tol[4]}')
ax.plot(tol_exps, counter_Tol_5[0], '-*', c='#800000', label=f'AM & {all_abs_tol[5]} & {all_rel_tol[5]}')
ax.plot(tol_exps, counter_Tol_5[1], '-*', c='#252525', label=f'BDF & {all_abs_tol[5]} & {all_rel_tol[5]}')
...
ax1.axhline(0.72, 0.11, 0.12, c='#fee5d9', marker=0)
ax1.axhline(0.72, 0.12, 0.12, c='#fee5d9', marker='*')
ax1.axhline(0.72, 0.12, 0.13, c='#fee5d9', marker=1)
ax1.axhline(0.62, 0.11, 0.12, c='#fdae61', marker=0)
ax1.axhline(0.62, 0.12, 0.12, c='#fdae61', marker='*')
ax1.axhline(0.62, 0.12, 0.13, c='#fdae61', marker=1)
ax1.axhline(0.52, 0.11, 0.12, c='#f46d43', marker=0)
ax1.axhline(0.52, 0.12, 0.12, c='#f46d43', marker='*')
ax1.axhline(0.52, 0.12, 0.13, c='#f46d43', marker=1)
ax1.axhline(0.42, 0.11, 0.12, c='#d73027', marker=0)
ax1.axhline(0.42, 0.12, 0.12, c='#d73027', marker='*')
ax1.axhline(0.42, 0.12, 0.13, c='#d73027', marker=1)
ax1.axhline(0.32, 0.11, 0.12, c='#a50026', marker=0)
ax1.axhline(0.32, 0.12, 0.12, c='#a50026', marker='*')
ax1.axhline(0.32, 0.12, 0.13, c='#a50026', marker=1)
ax1.axhline(0.22, 0.11, 0.12, c='#800000', marker=0)
ax1.axhline(0.22, 0.12, 0.12, c='#800000', marker='*')
ax1.axhline(0.22, 0.12, 0.13, c='#800000', marker=1)
# second column
...
ax1.axhline(0.22, 0.31, 0.32, c='#2E2D66', marker=0)
ax1.axhline(0.22, 0.32, 0.32, c='#252525', marker='*')
ax1.axhline(0.22, 0.32, 0.33, c='#252525', marker=1)
# third column
...
ax1.text(0.5, 0.18, r'$10^{-14}$', fontsize=10)
# fourth column
...
ax1.text(0.8, 0.18, r'$10^{-14}$', fontsize=10)
