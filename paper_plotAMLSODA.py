...
for iFile in range(0, int(len(list_directory_general)/2)):
    am_split = list_directory_general[iFile].split('_')
    if am_split[0] == '1' and am_split[1] == '2'and am_split[2] == '9':
        list_directory_am.append(list_directory_general[iFile])
...
print('am_zero_x_smallest: ' + str(sorted(am_zero_x)[0]))
print('am_zero_y_smallest: ' + str(sorted(am_zero_y)[0]))
...
print('am_zero_x_largest: ' + str(sorted(am_zero_x, reverse=True)[0]))
print('am_zero_y_largest: ' + str(sorted(am_zero_y, reverse=True)[0]))
...
colors_2 = [(1, 0.9, 0.6, 0.3), (0.96, 0.41, 0, 1)]
...
# orange
grid_orange = np.vstack([np.log(am_lsoda_x), np.log(am_lsoda_y)])
...
am_lsoda_x, am_lsoda_y, kde_orange = np.array(am_lsoda_x)[ids_orange], np.array(am_lsoda_y)[ids_orange], np.array(kde_orange)[ids_orange]
...
plt2 = ax.scatter(am_lsoda_x, am_lsoda_y, s=marker_size, c=kde_orange, cmap=cm_2, label='AM faster: ' + str(round(len(am_lsoda_x)/len(lsoda_tsv_file['t_intern_ms'])*100/len(list_directory_lsoda), 2)) + ' %', zorder=10, clip_on=False, alpha=alpha)
plt3 = ax.scatter(equal_x, equal_y, s=marker_size, c='grey', zorder=10, clip_on=False, alpha=alpha)
plt4 = ax.scatter(lsoda_zero_x, lsoda_zero_y, c='#4c3340', cmap=cm_2, marker='D', s=marker_size, facecolors='none', edgecolors='orange', zorder=10, clip_on=False)
plt5 = ax.scatter(am_zero_x, am_zero_y, c='orange', cmap=cm_1, s=marker_size, facecolors='none', edgecolors='#4c3340', marker='D', zorder=10, clip_on=False)
...
ax.plot(0.4, 37000, 'o', fillstyle='full', c='orange', markersize=marker_size)
...
# plot text 'E'
ax.text(-0.18, 1, 'E', fontsize=fontsize + 5, transform=ax.transAxes)
...
plt.plot([-np.log10(200), -np.log10(200)], [0.7, 2000], '--', color='grey',
...
plt.scatter(ratio_lsoda_zero, num_states_lsoda_zero,
            s=marker_size, c='orange', cmap=cm_2, alpha=alpha, marker='D',
            zorder=10, clip_on=False)
...
ax2.set_xlim((-np.log10(200), np.log10(200)))
ax2.set_yticks([])
plt.minorticks_off()
...
plt.xticks([-np.log10(200), -2, -1, 0, 1, 2, np.log10(200)],
           ['', '$10^{-2}$', '$10^{-1}$', '1', '$10^1$', '$10^2$',
            ''], fontsize=fontsize)
ax2.text(-np.log10(300), 20, 'AM failed', fontsize=fontsize, rotation=90, ha='center')
ax2.text(np.log10(300), 20, 'LSODA failed', fontsize=fontsize, rotation=-90, ha='center')

# plot text 'F'
ax.text(-0.08, 1, 'F', fontsize=fontsize + 5, transform=ax2.transAxes)
